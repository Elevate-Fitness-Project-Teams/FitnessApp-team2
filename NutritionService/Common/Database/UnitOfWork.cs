...
public class UnitOfWork : IUnitOfWork
{
    ...
    private async Task<T> ExecuteInternalAsync<T>(Func<Task<T>> action, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            // Handle specific Result implementations if needed. 
            // In NutritionService, we usually rely on IsFailure but for generic transaction we just commit if no exception.
            // If the user wants to rollback on explicit failure, we check IsFailure.
            if (result is NutritionService.Common.Result r && r.IsFailure)
            {
                shouldRollback = true;
            }
